...
export default function ProductDetailsNotif({ route, navigation }) {
  ...
  useEffect(() => {
    console.log('🔍 ProductDetailsNotif received params:', { 
      ...
      initialDataKeys: initialProductData ? Object.keys(initialProductData) : []
    });
  ...
  const processNotificationData = () => {
    try {
      ...
      if (initialProductData && Object.keys(initialProductData).length > 0) {
        ...
        let processedImages = [];
        if (initialProductData.images && Array.isArray(initialProductData.images)) {
          processedImages = initialProductData.images.map(img => {
        ...
        console.log('📊 Processed product:', {
          ...
          imageCount: processedProduct.images.length
        });
